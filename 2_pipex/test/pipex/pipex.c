/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   pipex.c                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: junhylee <junhylee@student.42seoul.kr>     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/11 18:27:25 by junhylee          #+#    #+#             */
/*   Updated: 2024/01/15 17:51:59 by junhylee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pipex.h"

void	first_prc(char *file, int *pipe_fd, char **cmd, char **envp)
{
	int	fd_file1;

	fd_file1 = open(file, O_RDWR);
	if (fd_file1 == -1
		|| dup2(fd_file1, 0) == -1
		|| dup2(pipe_fd[1], 1) == -1
		|| close(pipe_fd[0]) == -1
		|| close(fd_file1) == -1
		|| execve(cmd[0], cmd, envp) == -1)
		ft_error(errno);
}

void	last_prc(int argc, char **argv, int *pipe_fd, char **envp)
{
	int		fd_file;
	char	**cmd;
	pid_t	pid;

	if (access(argv[argc - 1], F_OK) == 0)
		fd_file = open(argv[argc - 1], O_TRUNC | O_WRONLY);
	else
		fd_file = open(argv[argc - 1], O_CREAT | O_WRONLY, 0777);
	if (fd_file == -1)
		ft_error(errno);
	cmd = ft_split(argv[argc - 2], envp);
	pid = get_fork_pid();
	if (pid == 0)
	{
		// if (fd_file == -1)
		// 	ft_error(errno);
		if (dup2(pipe_fd[0], 0) == -1)
			ft_error(errno);
		if (dup2(fd_file, 1) == -1)
			ft_error(errno);
		if (close(pipe_fd[1]) == -1 || close(fd_file) == -1)
			ft_error(errno);
		if (execve(cmd[0], cmd, envp) == -1)
			ft_error(errno);
	}
	if (pid > 0)
		split_frees(cmd);
}

void	ft_prc_mid(int *in_pipe, int *out_pipe, char **cmd, char **envp)
{
	if (dup2(in_pipe[0], 0) == -1
		|| dup2(out_pipe[1], 1) == -1
		|| close(in_pipe[1]) == -1
		|| close(out_pipe[0]) == -1
		|| execve(cmd[0], cmd, envp) == -1)
		ft_error(errno);
}

void	make_mid_prc(int prc_cnt, int **pipe_fd, char **argv, char **envp)
{
	int		i;
	pid_t	pid;
	char	**cmd;

	i = 0;
	while (i < prc_cnt - 2)// first, last process 제외
	{
		cmd = ft_split(argv[i + 3], envp);
		pid = get_fork_pid();
		if (pid == 0)
			ft_prc_mid(pipe_fd[i], pipe_fd[i + 1], cmd, envp);//first prc가 pipe_fd[0]이니까 그 다음부터
		if (pid > 0)
		{
			if ((close(pipe_fd[i + 1][0]) == -1) ||
				(close(pipe_fd[i + 1][1]) == -1))
				ft_error(errno);
			split_frees(cmd);
		}
		i++;
	}
}

int	main(int argc, char **argv, char **envp)
{
	pid_t	first_pid;
	int		prc_cnt;
	int		**pipe_fd;
	char	**cmd;

	pipe_fd = init_pipe(argc, argv, &prc_cnt);
	cmd = ft_split(argv[2], envp);
	first_pid = get_fork_pid();
	if (first_pid == 0)
		first_prc(argv[1], pipe_fd[0], cmd, envp);
	else if (first_pid > 0)
	{
		make_mid_prc(prc_cnt, pipe_fd, argv, envp);
		last_prc(argc, argv, pipe_fd[prc_cnt - 2], envp);
		if (close(pipe_fd[0][0]) == -1 || close(pipe_fd[0][1] == -1))
			ft_error(errno);
		ft_wait_prcs(2);
		split_frees(cmd);
	}
}
